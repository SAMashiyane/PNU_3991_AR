...
public class MainCode {
	...
	public static Datacenter createDatacenter(String name,Class<? extends Datacenter> datacenterClass,List<PowerHost> hostList,VmAllocationPolicy vmAllocationPolicy)
		...
		try {
			datacenter = datacenterClass.getConstructor(String.class,
                                DatacenterCharacteristics.class,VmAllocationPolicy.class,List.class,Double.TYPE).newInstance(name,characteristics,vmAllocationPolicy,
	...
	/**
	 *  با استفاده از این متد زمان شروع عملیات در دیتاسنترددریافت مگردد
	 * 
	 * @param vms the vms
	...
	public static void printResults(PowerDatacenter datacenter,List<Vm> vms,double lastClock,String experimentName,boolean outputInCsv,String outputFolder)
		...
		if (outputInCsv) {
			...
			data.append(String.format("%d", numberOfHosts)).append(delimeter);
                        StringBuilder append = data.append(String.format("%d", numberOfVms)).append(delimeter);
			data.append(String.format("%.2f", totalSimulationTime)).append(delimeter);
                        StringBuilder append1 = data.append(String.format("%.5f", energy)).append(delimeter);
			data.append(String.format("%d", numberOfMigrations)).append(delimeter);
			data.append(String.format("%.10f", sla)).append(delimeter);
                        StringBuilder append2 = data.append(String.format("%.10f", slaTimePerActiveHost)).append(delimeter);
                        StringBuilder append3 = data.append(String.format("%.10f", slaDegradationDueToMigration)).append(delimeter);
                        StringBuilder append4 = data.append(String.format("%.10f", slaOverall)).append(delimeter);
                        StringBuilder append5 = data.append(String.format("%.10f", slaAverage)).append(delimeter);
			...
			if (datacenter.getVmAllocationPolicy() instanceof PowerVmAllocationPolicyMigrationAbstract) {
				...
				data.append(String.format("%.5f", executionTimeVmSelectionStDev)).append(delimeter);
                                StringBuilder append6 = data.append(String.format("%.5f", executionTimeHostSelectionMean)).append(delimeter);
                                StringBuilder append7 = data.append(String.format("%.5f", executionTimeHostSelectionStDev)).append(delimeter);
                                StringBuilder append8 = data.append(String.format("%.5f", executionTimeVmReallocationMean)).append(delimeter);
	...
	public static void printCloudletList(List<Cloudlet> list) {
		...
		String indent = "\t";
		Log.printLine();
		Log.printLine("========== resource management with load balancing in Datacenters OUTPUT ==========");
		Log.printLine("Cloudlet ID" + indent+ indent + "STATUS" + indent + "Resource ID" + indent + "VM ID" + indent
		...
		for (int i = 0; i < size; i++) {
			...
			if (cloudlet.getCloudletStatus() == Cloudlet.SUCCESS) {
				Log.printLine(indent + "RM: " + indent + indent 
                ...
                Log.printLine(" Resource Management In Cloud Is Finished ...");
